...
- HDRP + deferred: 
  ...
  > 这里考虑的是直接把真实物理量存入Texture中。因为如果存入的是曝光后的结果，后续采样后，还需要重新进行逆曝光才能参与物理计算。
  > 但这里除以1000有导致了个问题： unity会将小于0.001的值视为0 -> URP上较小的光强直接被忽略了。
...
- unity中不是Cubemap/lightmap的HDR图统一使用RGBM进行存储？ 前者各自受project settings中的影响